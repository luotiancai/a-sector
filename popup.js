...
function analyzeNews() {
  ...
  const prompt = `你是A股事件驱动交易员。判断标准只有一个：这条新闻会不会让交易员立刻去买或卖某个板块？

【必须 skip=true 的情况】
- 企业日常经营行为：促销活动、以旧换新、满减优惠、限时折扣、普通新品发布（无颠覆性技术）、企业自掏腰包的补贴/优惠券/折扣（区别于政府补贴政策）
- 企业常规公告：成立普通子公司、签小额日常合同、高管正常变动、例行分红
- 境外无关社会事件：他国事故/犯罪/选举，与A股无直接传导路径的
- 纯行情播报：境外指数涨跌、商品价格日常小幅波动（无具体涨跌数字或涨跌幅明显低于1%）
- 模糊信息：无法判断对任何板块影响的

【保留并判断的情况——必须有明确的板块传导路径】
利好(bullish) 例：
  - 国家/政府级产业政策出台（财政补贴/扶持/准入放开）→ 受益行业（注意：企业自己搞的促销补贴不算）
  - 供给端冲击（矿难/工厂爆炸/产区灾害）→ 对应大宗商品板块
  - 地缘冲突升级/战争 → 军工/石油/黄金
  - 央行降准降息/超预期宽松 → 银行/地产/券商
  - 企业重大事件：国家级大单中标、核心技术突破获批、被纳入重要指数
  - 关键大宗商品单日明显上涨（碳酸锂/铜/煤/原油/稀土等，有具体涨价数字）→ 对应生产板块利好

利空(bearish) 例：
  - 加征关税/贸易制裁 → 受损出口行业
  - 行业强监管/整治（罚款/叫停）→ 对应板块
  - 需求端骤降数据（PMI大幅低于预期、消费数据崩塌）
  - 企业重大负面：监管立案/巨额罚款/主力产品被叫停
  - 关键大宗商品单日明显下跌（有具体跌价数字）→ 对应生产板块利空

【特别注意】
- 新闻里同时出现"震荡""偏弱"等模糊词和具体涨跌数字时，以数字为准判断方向，不要被文字描述干扰
- "补贴"一词需判断主体：政府/国家发的→可能利好；企业自己给消费者打折→skip

中性(neutral)：影响存在但方向不明，或属于单一公司的重大事件但不扩散到板块

遇到具体公司名，根据主营业务推断板块（宁德时代→电池；中芯国际→半导体；中国石油→石油石化）

可选板块：${sectors}

严格只返回JSON数组，不要任何其他文字：
[{"i":0,"skip":false,"s":"bullish","tags":["电力"],"r":"核电审批加速"},...]
s: bullish/bearish/neutral，skip=true 时 s/tags/r 填空字符串
